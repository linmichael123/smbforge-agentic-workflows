...
- **Telegram owner bot with 27 tool declarations** — an LLM-native command surface (approve/decline/edit bookings, view schedule, broadcast campaigns, manage leads) built directly on the Telegram Bot API.
...
The production system's core agents were extracted into standalone, deploy-ready templates under [`products/`](products/):

- [`products/ai-scheduler/`](products/ai-scheduler/) — Google Calendar conversational booking agent (`list_free_slots`, `book_appointment`, `cancel_appointment` via LLM tool calls)
- [`products/ai-invoicing/`](products/ai-invoicing/) — Stripe conversational invoicing agent with pre-wired webhook handling
- [`products/ai-ordering/`](products/ai-ordering/) — Conversational product catalog & stateful cart manager

Each module is SDK-free (raw `fetch()` integrations), MIT licensed, and deployable on Cloudflare's free tier.
...
├── products/
│   ├── ai-scheduler/          ← Extracted booking agent template + curl examples
│   ├── ai-invoicing/          ← Extracted Stripe invoicing agent template + curl examples
│   └── ai-ordering/           ← Extracted catalog/cart agent template + curl examples
├── examples/
